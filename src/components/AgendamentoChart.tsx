...
export type AreaPoint = { date: string; agendamentos: number }
...
export function AgendamentoChart({ range, hideSelectors = false }: { range?: { start: Date; end: Date }; hideSelectors?: boolean }) {
  ...
  React.useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const ag = (Array.isArray(agUnknown) ? agUnknown : []) as Array<{ dataHoraInicio?: string; data_hora?: string }>
        ...
        const points: AreaPoint[] = Array.from(byDay.entries())
          ...
          .map(([date, count]) => ({ date, agendamentos: count }))
        setData(points)
      } catch {}
    }
    void fetchData()
